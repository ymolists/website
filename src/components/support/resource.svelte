...
<div
  class="flex flex-col justify-between items-center max-w-xs lg:max-w-sm mb-small bg-white py-small px-x-small sm:py-small  rounded-4xl shadow-normal"
>
  <div>
    {#if icon}
      <img src={icon} alt={title} class="h-12 mb-x-small mx-auto" />
    {/if}
    <h3 class="mb-micro">{title}</h3>
    <p>{text}</p>
  </div>
